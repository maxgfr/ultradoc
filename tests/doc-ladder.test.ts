...
import { extractDocument, enabledDocExtractors, resetDocLadderCache } from "../src/sources/doc/ladder.js";
import { runWithInput, ANYDOC_SPEC } from "../src/sources/pdf/exec.js";
...
// registry.
vi.mock("../src/sources/pdf/exec.js", () => ({
...
describe("enabledDocExtractors", () => {
  it("defaults to the full ladder, strongest first", () => {
    vi.stubEnv("ULTRADOC_DOC_ENGINE", undefined);
    expect(enabledDocExtractors()).toEqual(["anydoc", "firecrawl"]);
  });

  it("honours ULTRADOC_DOC_ENGINE by running exactly that rung", () => {
    vi.stubEnv("ULTRADOC_DOC_ENGINE", "firecrawl");
  ...
  it("empties the ladder on ULTRADOC_DOC_ENGINE=none", () => {
    vi.stubEnv("ULTRADOC_DOC_ENGINE", "none");
    expect(enabledDocExtractors()).toEqual([]);
  });

  it("drops the rung that needs an implicit install under ULTRADOC_NO_NPX", () => {
    vi.stubEnv("ULTRADOC_DOC_ENGINE", undefined);
    vi.stubEnv("ULTRADOC_NO_NPX", "1");
    expect(enabledDocExtractors()).toEqual(["firecrawl"]);
  });

  it("ignores an unknown engine name rather than emptying the ladder", () => {
    vi.stubEnv("ULTRADOC_DOC_ENGINE", "nope");
    expect(enabledDocExtractors()).toEqual(["anydoc", "firecrawl"]);
  });
