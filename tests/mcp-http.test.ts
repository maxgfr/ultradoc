...
beforeAll(async () => {
  running = await startHttpServer(ultradocAdapter(), { port: 0 });
});
afterAll(async () => {
  await running.close();
});
...
describe("the JSON-RPC contract", () => {
  it("initializes and identifies itself, issuing no session id", async () => {
    ...
    expect(body.result.serverInfo.name).toBe("ultradoc");
  });
  ...
  it("lists tools, and answers a batch with an array", async () => {
    ...
    expect(single.json().result.tools.map((t: { name: string }) => t.name)).toContain("ultradoc_search");
  ...
  it("runs a real tool call end to end", async () => {
    const res = await post({
      body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultradoc_read", arguments: { repo: LIB, path: "src/retry.ts", end_line: 3 } } },
    });
    expect(res.status).toBe(200);
  ...
  it("returns a failing tool as 200 + isError, never as an HTTP 5xx", async () => {
    ...
    const res = await post({
      body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultradoc_read", arguments: { repo: LIB, path: "/etc/passwd" } } },
    });
    expect(res.status).toBe(200);
...
describe("statelessness under concurrency", () => {
  it("keeps two overlapping requests on different protocol versions from reading each other's", async () => {
    ...
    const pick = (r: { json: () => { result: { tools: { name: string }[] } } }) =>
      r.json().result.tools.find((t) => t.name === "ultradoc_read") as Record<string, unknown>;
...
describe("protocol version negotiation over HTTP", () => {
  it("accepts a supported version header", async () => {
    ...
    const read = res.json().result.tools.find((t: { name: string }) => t.name === "ultradoc_read");
...
describe("DNS-rebinding defense", () => {
  ...
  it("honours an explicit allow-list", async () => {
    const alt = await startHttpServer(ultradocAdapter(), { port: 0, allowOrigin: ["https://app.example.com"] });
    try {
  ...
  it("refuses to bind a non-loopback address without --allow-remote", async () => {
    await expect(startHttpServer(ultradocAdapter(), { port: 0, bind: "0.0.0.0" })).rejects.toThrow(/refusing to bind/);
  });
});
