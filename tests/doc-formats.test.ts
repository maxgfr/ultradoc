...
import { docFormatForUrl, docFormatForContentType, DOC_EXTENSIONS } from "../src/sources/doc/formats.js";
// fetch.ts keeps its PDF test as a private constant, so this mirrors it. If the
// two ever drift, the partition below is exactly what catches it.
const looksLikePdfUrl = (u: string) => /\.pdf($|[?#])/i.test(u);
import { ANYDOC_SPEC, PDF_INSPECTOR_SPEC } from "../src/sources/pdf/exec.js";
