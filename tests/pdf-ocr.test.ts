...
import { ocrPdf, ocrTools, ocrBudgetLeft, resetOcrBudget } from "../src/sources/pdf/ocr.js";
...
beforeEach(() => vi.stubEnv("ULTRADOC_OCR_MAX", "3"));
...
describe("ocrPdf", () => {
  ...
  it("honours ULTRADOC_OCR_LANG", async () => {
    vi.stubEnv("ULTRADOC_OCR_LANG", "fra+eng");
    toolsPresent();
    await ocrPdf(PDF);
...
describe("the per-process OCR budget", () => {
  ...
  it("stops after ULTRADOC_OCR_MAX documents", async () => {
    vi.stubEnv("ULTRADOC_OCR_MAX", "2");
    toolsPresent();
