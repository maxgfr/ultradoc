...
const TRACKED = [
  // The index snapshot is codeindex walking the golden fixture: callSites,
  // configFiles, resolved imports. A newer engine sees more of the same repo,
  // so this moves on a bump and must not be a hard red. The `orchestrate`
  // snapshot beside it is the RUNBOOK this skill writes itself — deliberately
  // NOT listed, so it stays a hard red.
  "tests/__snapshots__/golden-index.test.ts.snap",
];
