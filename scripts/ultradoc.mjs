...
import { existsSync as existsSync21, realpathSync as realpathSync4 } from "fs";
...
async function Module2(moduleArg = {}) {
    ...
    if (Module["instantiateWasm"]) {
      return new Promise((resolve33, reject) => {
        Module["instantiateWasm"](info2, (mod, inst) => {
          resolve33(receiveInstance(mod, inst));
        });
      });
    }
  ...
  if (runtimeInitialized) {
    ...
    moduleRtn = new Promise((resolve33, reject) => {
      readyPromiseResolve = resolve33;
...
function topDocComment(content) {
  ...
  const text = collected2.filter((l) => l && !isDirective(l) && !isBanner(l)).join(" ").replace(/\s+/g, " ").trim();
...
function readPersistedIndex(repo, indexDir2 = INDEX_DIR) {
  let parsed;
  try {
    parsed = JSON.parse(readFileSync3(join3(repo, indexDir2, "cache.json"), "utf8"));
  } catch {
    return void 0;
  }
...
function preloadArtifacts(repo, scan2, meta, indexDir2 = INDEX_DIR) {
  ...
  const dir = join3(repo, indexDir2);
  let graphBytes;
  let symbolsBytes;
  try {
...
function preloadSession(repo, opts, indexDir2 = INDEX_DIR) {
  const persisted = readPersistedIndex(repo, indexDir2);
  if (!persisted) return void 0;
  const scan2 = scanRepo(repo, { ...opts, cache: persisted.cacheMap });
  return { scan: scan2, cacheMap: toCacheMap(scan2), arts: preloadArtifacts(repo, scan2, persisted.meta, indexDir2) };
}
var INDEX_DIR;
...
function searchIndex(scan2, query4, opts = {}) {
  const terms = [];
  const seen = /* @__PURE__ */ new Set();
  for (const kw of keywords(query4)) {
...
function bm25TrigramsFor(scan2) {
  const c2 = cacheFor(scan2);
  const bm252 = c2.bm25 ??= { docs: buildDocs(scan2) };
  return bm252.trigrams ??= buildTrigramIndex(bm252.docs);
}
...
function bm25StemsFor(scan2) {
  const c2 = cacheFor(scan2);
  const bm252 = c2.bm25 ??= { docs: buildDocs(scan2) };
  return bm252.stems ??= buildStemIndex(bm252.docs);
}
...
async function encodeQueryViaEndpoint(query4, opts = {}) {
  const [vec] = await embedViaEndpoint([query4], opts);
...
function memoizedEmbedModel(modelDir2) {
  let stat;
  try {
    stat = statSync5(join18(modelDir2, "model.json"));
  } catch {
    return void 0;
  }
  const key = `${modelDir2}:${stat.mtimeMs}:${stat.size}`;
  if (embedModelCache && embedModelCache.key === key) return embedModelCache.model;
  const model = loadEmbedModel(modelDir2);
...
async function callTool(name2, args2, defaultRepo) {
  ...
  if (name2 === "search") {
    ...
    if (args2.semantic === true) {
      ...
      if (endpoint) {
        try {
          ...
          const results2 = searchIndex(scan2, query4, { limit, fuzzy, ...rankOpt });
          return JSON.stringify(
      ...
      const modelDir2 = resolveEmbedModelDir(repo);
      const model = modelDir2 ? memoizedEmbedModel(modelDir2) : void 0;
      if (model) {
        const index = await memoizedEmbeddingIndex(
          { mode: "static", identity: `${modelDir2}#${model.modelId}`, scan: scan2 },
      ...
      const results = searchIndex(scan2, query4, { limit, fuzzy, ...rankOpt });
      return JSON.stringify(
    ...
    return JSON.stringify(searchIndex(scan2, query4, { limit, fuzzy, ...rankOpt }), null, 2);
  }
  if (name2 === "embed_status") {
    ...
    const status = {
      ...
      mode,
      model: model ? { present: true, dir: modelDir2, modelId: model.modelId, dim: model.dim, vocabSize: model.vocabSize } : { present: false },
...
`;
  try {
    const outputs = await Promise.all(
      shards.map(
        (jobsForShard) => new Promise((resolve33, reject) => {
...
function extractTags(ext, content) {
  ...
  const query4 = queryFor(key, language);
  if (!query4) return [];
  let tree = null;
  try {
    tree = parser2.parse(content);
...
`);
  ...
  const markerPath3 = join6(dirname3(cacheDir), `${ENGINE_VERSION}.sha256`);
  if (existsSync4(runtime) && expected && existsSync4(markerPath3)) {
    let marker = "";
    try {
      marker = readFileSync5(markerPath3, "utf8").trim();
    } catch {
    }
    if (marker === expected) {
...
`
  ...
  try {
    ...
    tmp = void 0;
    if (expected) writeFileSync(markerPath3, expected + "\n");
  } catch (e) {
    if (tmp) {
      try {
...
async function runCli(rawArgv) {
  ...
  const indexDir2 = flags2.indexDir ?? INDEX_DIR;
  ...
  const tryPreload = () => {
    ...
    const p = preloadSession(flags2.repo, scanOptions(flags2, precomputedWalk), indexDir2);
  ...
  if (cmd === "index") {
    ...
    const modelDir2 = resolveEmbedModelDir(flags2.repo);
    const model = modelDir2 ? loadEmbedModel(modelDir2) : void 0;
...
// src/clone.ts
import { existsSync as existsSync10, mkdirSync as mkdirSync5, renameSync as renameSync3 } from "fs";
...
function envIntRaw(name2, def, min = 1) {
...
var LIMITS = {
  maxFiles: envIntRaw("ULTRADOC_MAX_FILES", 2e4),
  // files walked/indexed
  maxFileBytes: envIntRaw("ULTRADOC_MAX_FILE_BYTES", 1048576),
  // per-file read cap
  symbolsPerFile: envIntRaw("ULTRADOC_MAX_SYMBOLS_PER_FILE", 400),
  ...
  releasesFetched: envIntRaw("ULTRADOC_MAX_RELEASES", 20),
  // GitHub releases fetched
  docPackages: envIntRaw("ULTRADOC_MAX_DOC_PACKAGES", 6),
  // monorepo packages given doc sections
  docModules: envIntRaw("ULTRADOC_MAX_DOC_MODULES", 5),
  // subsystems given their own doc section
  verifyPairs: envIntRaw("ULTRADOC_MAX_VERIFY", 40),
  // claim↔evidence pairs (CLI --max-verify wins)
  embedChunks: envIntRaw("ULTRADOC_MAX_CHUNKS", 800),
  // semantic chunks embedded per repo
  embedConcurrency: envIntRaw("ULTRADOC_EMBED_CONCURRENCY", 4)
  // parallel embed requests
};
function extdocsTtlMs() {
  return envIntRaw("ULTRADOC_EXTDOCS_TTL_HOURS", 168) * 36e5;
}
...
import { existsSync as existsSync22, mkdirSync as mkdirSync4, readdirSync as readdirSync4, rmSync as rmSync22, statSync as statSync6 } from "fs";
import { tmpdir as tmpdir22 } from "os";
import { basename as basename3, join as join23, resolve as resolve3 } from "path";
import { spawnSync as spawnSync22 } from "child_process";
import { existsSync as existsSync32, mkdirSync as mkdirSync22, readFileSync as readFileSync22, writeFileSync as writeFileSync22 } from "fs";
import { tmpdir as tmpdir3 } from "os";
import { dirname as dirname5, join as join32 } from "path";
import { mkdirSync as mkdirSync32, renameSync as renameSync2, unlinkSync, writeFileSync as writeFileSync32 } from "fs";
import { readFileSync as readFileSync42 } from "fs";
import { existsSync as existsSync52 } from "fs";
import { join as join72, resolve as resolve22 } from "path";
import { join as join62 } from "path";
import { existsSync as existsSync62, readdirSync as readdirSync32, readFileSync as readFileSync52, realpathSync as realpathSync2, statSync as statSync32 } from "fs";
import { basename as basename32, dirname as dirname22, join as join82, resolve as resolve32, sep as sep3 } from "path";
...
async function ocrPdf(bytes) {
  ...
  const dir = mkdtempSync2(join22(tmpdir2(), `${brand().name}-ocr-`));
  try {
    const input = join22(dir, "in.pdf");
    const output = join22(dir, "out.pdf");
...
function charsetFromHtml(head) {
  const window2 = head.slice(0, 4096);
  const direct = /<meta[^>]+charset\s*=\s*["']?([a-z0-9_:.+-]+)/i.exec(window2);
  if (direct) return direct[1].toLowerCase();
  const httpEquiv = /<meta[^>]+http-equiv\s*=\s*["']?content-type["']?[^>]*content\s*=\s*["'][^"']*charset\s*=\s*([a-z0-9_:.+-]+)/i.exec(window2);
...
function excerptWindows(text, question, opts = {}) {
  ...
  for (let i2 = 0; i2 < lines.length; i2++) {
    ...
    for (const m of matchers) {
      const cov = m.matchLine(lines[i2]).size;
      if (cov > score) score = cov;
    }
    if (score > 0) hits.push({ anchor: i2, score });
  }
  hits.sort((a, b) => b.score - a.score || a.anchor - b.anchor);
  const take = hits.length ? hits : [{ anchor: 0, score: 0 }];
  const out2 = [];
  for (const h of take) {
    if (out2.length >= perDoc) break;
    const start2 = Math.max(0, h.anchor - before);
    const end = Math.min(lines.length, h.anchor + after);
    if (out2.some((w) => start2 < w.end && end > w.start)) continue;
    const snippet = lines.slice(start2, end).join("\n").slice(0, maxChars);
    if (!snippet.trim()) continue;
    const heading = nearestHeading(lines, h.anchor);
    out2.push({ start: start2, end, anchor: h.anchor, score: h.score, ...heading ? { heading } : {}, snippet });
  }
  return out2;
}
function slugify2(input, opts = {}) {
...
function looksLikeFirecrawl(contentType, body2) {
  if (/firecrawl/i.test(body2.slice(0, 4096))) return true;
  ...
  if (!p) {
    p = (async () => {
      ...
      try {
        ...
        return explicit || looksLikeFirecrawl(res.headers.get("content-type"), body2);
      } catch {
        return false;
      } finally {
        clearTimeout(t);
...
async function postJson(base, path, body2, timeoutMs) {
  const headers = authHeaders();
  const first = await httpJson("POST", `${base}${apiPrefix(base)}${path}`, body2, { timeoutMs, headers });
  if (first.status !== 404 || apiPrefix(base) !== "/v2") return first;
  prefixCache.set(base, "/v1");
  return httpJson("POST", `${base}/v1${path}`, body2, { timeoutMs, headers });
}
...
async function searchViaFirecrawl(query4, limit, opts = {}) {
  const base = firecrawlBase(opts);
  ...
  const r = await postJson(base, "/search", { query: query4, limit, sources: ["web"] }, SEARCH_TIMEOUT_MS);
  if (!r.ok) {
...
function sliceToMatchingClose(html, start2, tag) {
  ...
  while (m = re.exec(html)) {
    if (m[0][1] === "/") {
      if (--depth === 0) return html.slice(start2, m.index);
    } else {
      depth++;
    }
  }
...
function extractMainHtml(html) {
  ...
  for (const c2 of candidates.slice(1)) {
    const len = visible(c2);
    if (len > bestLen) {
      best = c2;
      bestLen = len;
    }
...
function toResult(status, stdout, stderr, err2) {
  const missing = err2?.code === "ENOENT";
  return {
    ok: !missing && status === 0,
    status: status ?? (missing ? 127 : 1),
    stdout,
    stderr: stderr || (err2 ? err2.message : ""),
...
function shAsync(cmd, args2, opts = {}) {
  ...
  return new Promise((resolve42) => {
    ...
    const child = spawn2(cmd, args2, { cwd: opts.cwd, env: opts.env ?? process.env, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    child.stdout?.on("data", (d) => {
...
function repoCacheRoot() {
  return env("REPO_DIR") ?? brand().repoDir ?? join23(tmpdir22(), brand().name, "repos");
}
...
function resolveRepo(raw) {
  ...
  if (trimmed) {
    ...
    if (existsSync22(asPath) && statSync6(asPath).isDirectory()) {
      return { raw: trimmed, host: "local", isLocal: true, slug: `local-${slugify2(`${basename3(asPath)}-${asPath}`)}` };
    }
  }
  ...
  if (scp) {
    ...
    return { raw: trimmed, host: "generic", isLocal: false, slug: slugify2(trimmed) || "seed" };
  }
...
async function ensureClone(ref, opts = {}) {
  ...
  const dir = join23(repoCacheRoot(), ref.slug);
  const cloned = existsSync22(join23(dir, ".git"));
  ...
  if (!first.ok) {
    if (existsSync22(dir)) {
      try {
        rmSync22(dir, { recursive: true, force: true });
      } catch (e) {
  ...
  if (!existsSync22(dir) || readdirSync4(dir).length === 0) throw new Error(`clone produced an empty tree at ${dir}`);
  return dir;
}
...
async function ensureHistoryDepth(dir, opts = {}) {
  ...
  const out2 = await computeHistoryDepth(dir, opts);
  deepened.set(dir, out2);
  return out2;
}
async function computeHistoryDepth(dir, opts) {
  if (!have2("git")) return { ok: false, note: "git is not installed \u2014 no commit history available." };
...
function headCommit2(dir) {
  const r = sh2("git", ["-C", dir, "rev-parse", "HEAD"], { timeoutMs: 1e4 });
  return r.ok ? r.stdout.trim() || void 0 : void 0;
}
function originUrl(dir) {
  const r = sh2("git", ["-C", dir, "remote", "get-url", "origin"], { timeoutMs: 1e4 });
  return r.ok ? r.stdout.trim() || void 0 : void 0;
}
var MIN_ABBREV = 7;
...
function cacheRoot2() {
  return env("CACHE_DIR") ?? brand().cacheDir ?? join32(tmpdir3(), brand().name);
}
function ensureComposeMaterialized() {
  const base = join32(cacheRoot2(), "compose");
...
function writeIfChanged(path, content) {
  try {
    if (existsSync32(path) && readFileSync22(path, "utf8") === content) return;
    mkdirSync22(dirname5(path), { recursive: true });
    writeFileSync22(path, content);
  } catch {
  }
}
...
function defaultRun(cmd, args2, opts) {
  const res = spawnSync22(cmd, args2, {
    encoding: "utf8",
...
var STACKS = {
  ...
  semantic: {
    ...
    postUp: (file, run3) => {
      ...
      const pull = run3("docker", ["compose", "-f", file, "exec", "-T", "ollama", "ollama", "pull", model], { timeoutMs: MODEL_PULL_TIMEOUT_MS, capture: true });
  ...
  all: {
    ...
    postUp: (file, run3) => STACKS.semantic.postUp(file, run3)
  }
};
function combine(names) {
  ...
  return {
    ...
    postUp: (file, run3) => found.flatMap((x) => x.postUp?.(file, run3) ?? [])
  };
}
...
function stackControl(service, action, deps = {}) {
  ...
  if (action === "down") {
    const r = run3("docker", ["compose", "-f", file, ...profiles, "down"], { timeoutMs: DOWN_TIMEOUT_MS, capture: true });
    return { message: r.ok ? `${tag}: stopped.` : `${tag}: down failed.
${r.stderr}`, code: r.ok ? 0 : 1 };
  }
  if (action === "status") {
    const r = run3("docker", ["compose", "-f", file, ...profiles, "ps"], { timeoutMs: PS_TIMEOUT_MS, capture: true });
    return { message: r.ok ? r.stdout.trim() || `${tag}: no services running.` : `${tag}: status failed.
${r.stderr}`, code: 0 };
  }
  const pulled = run3("docker", ["compose", "-f", file, ...profiles, "pull"], { timeoutMs: pullTimeoutMs() });
  if (!pulled.ok) {
    return {
...
${pulled.stderr}` : ""),
  ...
  const up = run3("docker", ["compose", "-f", file, ...profiles, "up", "-d", "--wait"], { timeoutMs: UP_TIMEOUT_MS });
  if (!up.ok) return { message: `${tag}: up failed.${up.stderr ? `
${up.stderr}` : ""}`, code: 1 };
  return { message: [`${tag}: ${spec.summary}`, ...spec.postUp?.(file, run3) ?? []].join("\n"), code: 0 };
}
...
async function embed(texts, opts = {}) {
  ...
  for (let i2 = 0; i2 < texts.length; i2 += width) batches.push(texts.slice(i2, i2 + width));
  let note;
...
function extractClaimUnits(text, opts = {}) {
  ...
  const skip = (i22) => code[i22] === true || extra[i22] === true;
  ...
  while (i2 < lines.length) {
    ...
    if (/^\s*>/.test(line)) {
      ...
      while (i2 < lines.length && !skip(i2)) {
        if (!/^\s*>/.test(stripInlineCode(lines[i2]))) break;
        const dq = stored(lines[i2]).replace(/^\s*>\s?/, "").trim();
...
function citationTokensIn(text, isCitation2) {
  const masked = stripInlineCode(text);
  const out2 = [];
  for (const m of masked.matchAll(TOKEN_RE2)) {
    const tok = m[1].trim();
    if (isCitation2(tok) && !out2.includes(tok)) out2.push(tok);
  }
  return out2;
}
function collectCitations(text, isCitation2, opts = {}) {
  const grounding = [];
  for (const unit of extractClaimUnits(text, opts)) {
    for (const part of unitTexts(unit)) {
      for (const tok of citationTokensIn(part, isCitation2)) if (!grounding.includes(tok)) grounding.push(tok);
    }
  }
  const all = [];
  for (const m of text.matchAll(TOKEN_RE2)) {
    const tok = m[1].trim();
    if (isCitation2(tok) && !all.includes(tok)) all.push(tok);
  }
...
function toBatches(ids, batchSize) {
  ...
  const out2 = [];
  for (let i2 = 0; i2 < ids.length; i2 += width) out2.push(ids.slice(i2, i2 + width));
  return out2;
}
...
function emitWorkflowScript(phase, emission, runAbs, engineAbs, smallWorklist, constants = {}) {
  const cli = brand().cli;
  const scriptPath = join62(runAbs, "orchestration", `${phase.name}.workflow.mjs`);
  ...
  const script = [
    ...
    ...Object.entries(constants).map(([name2, value]) => `const ${name2} = ${JSON.stringify(value)}`),
    ``,
...
function runbookMd(phases, defs, runAbs, engineAbs, cli, preamble = []) {
  ...
  phases.forEach((ph, i2) => {
    const emission = defs[i2];
    lines.push(`## ${ph.name}`, ``);
    if (!ph.ready) {
    ...
    if (emission) {
      ...
      lines.push(
        `Fan out: \`Workflow({ scriptPath: "${join62(runAbs, "orchestration", `${ph.name}.workflow.mjs`)}" })\``,
...
function listPhases(runDir2, engineAbs, defs) {
  const run3 = resolve22(runDir2);
  return defs.map((def) => {
    const worklist = join72(run3, def.worklist);
    const parsed = readJsonSafe(worklist);
    const ids = def.ids(parsed, run3, engineAbs);
    ...
    return {
      ...
      items: ids?.length ?? 0,
      ids: ids ?? [],
      prerequisite: def.prerequisite(run3, engineAbs, parsed),
      ...ready ? { parsed } : {}
    };
  });
}
function orchestrateRun(runDir2, engineAbs, defs, contracts, opts = {}) {
  const run3 = resolve22(runDir2);
  if (!existsSync52(run3)) {
    return { exitCode: 2, written: [], notices: [], errors: [`run dir not found: ${run3}`], phases: [] };
  }
  const phases = listPhases(run3, engineAbs, defs);
  ...
  const orchDir = join72(run3, "orchestration");
  const agentsDir = join72(orchDir, "agents");
  ensureDir(join72(orchDir, "out"));
  ensureDir(agentsDir);
  const written = [];
  const notices = [];
  for (const [name2, content] of Object.entries(contracts(run3, engineAbs, phases))) {
    written.push(writeArtifact(join72(agentsDir, `${name2}.md`), content));
  }
  if (!opts.eco) {
    for (const ph of selected) {
      ...
      written.push(writeArtifact(join72(orchDir, `${ph.name}.workflow.mjs`), emitWorkflowScript(ph, def, run3, engineAbs, small, opts.constants)));
    }
  }
  written.push(writeArtifact(join72(orchDir, "RUNBOOK.md"), runbookMd(phases, defs, run3, engineAbs, brand().cli, opts.runbookPreamble)));
...
function parseArgs(argv, spec) {
  ...
  for (let i2 = 1; i2 < argv.length; i2++) {
    const arg = argv[i2];
    if (arg === "--") {
      positional.push(...argv.slice(i2 + 1));
      break;
    }
...
function resolveSkillRoot(moduleDir) {
  const here = moduleDir ?? dirname22(fileURLToPath3(import.meta.url));
  const name2 = brand().name;
  const candidates = [resolve32(here, ".."), resolve32(here, "..", "skills", name2), resolve32(here, "..", "..", "skills", name2)];
  return candidates.find((dir) => existsSync62(join82(dir, "SKILL.md")));
}
function listResources(moduleDir) {
  ...
  if (!existsSync62(refDir)) return out2;
  for (const file of readdirSync32(refDir).sort()) {
    if (!file.endsWith(".md")) continue;
    out2.push(describe(root, join82("references", file), `${skillName()} reference: ${basename32(file, ".md")}`));
  }
  return out2;
}
function readResource(uri, moduleDir) {
  ...
  const target = resolve32(root, rel2);
  ...
  if (!statSync32(targetReal).isFile()) throw new ResourceError(`not a file: ${uri}`);
  return { uri, mimeType: "text/markdown", text: readFileSync52(targetReal, "utf8") };
}
...
async function ensureRepoClone(ref, opts = {}) {
  if (!ref.isLocal) migrateLegacyClone(join24(cacheRoot(), ref.slug), ref.slug);
  return ensureClone(ref, opts);
}

// src/index/structural.ts
import { existsSync as existsSync11, mkdirSync as mkdirSync6, readFileSync as readFileSync12 } from "fs";
...
function scanOptions2(root, maxFiles) {
  return {
    ...
    // to skip its own.
    out: join25(root, CACHE_DIR_NAME)
  };
}
...
function discoverDocsUrl(repoDir, docFiles, configFiles, projectNames = []) {
  ...
  if (readme) {
    const text = readText(join26(repoDir, readme)).slice(0, 4e4);
    let m;
    const link = /\[([^\]]*)\]\((https?:\/\/[^)]+)\)/g;
  ...
  for (const cfg of configFiles) {
    ...
    const text = readText(join26(repoDir, cfg));
    if (!text) continue;
...
function resolveWorkspacePackage(packages, query4) {
...
function indexDir(root) {
  return join27(root, ".ultradoc");
}
function indexPath(root) {
  return join27(indexDir(root), "index.json");
}
...
function buildIndex(root, slug, opts = {}) {
  ...
  try {
    mkdirSync6(indexDir(root), { recursive: true });
...
function defaultRunDir(repoDir, d) {
  return join28(indexDir(repoDir), "runs", runId(d));
}
...
function writeDossier(dir, evidence, meta) {
  ...
  const evidenceJson = join28(dir, "evidence.json");
  const evidenceMd = join28(dir, "EVIDENCE.md");
  const metaJson = join28(dir, "meta.json");
...
function searchCode(root, ref, index, question, perSource, scope) {
  ...
  const candidates = [...files].filter((rel2) => lexical.has(rel2)).map((rel2) => {
    let len = 1e3;
    try {
      len = Math.max(1, statSync7(join29(root, rel2)).size / 5);
    } catch {
    }
  ...
  for (const f of scored) {
    ...
    const content = readText(join29(root, f.rel));
    if (!content) continue;
    const lines = content.split(/\r?\n/);
...
function codeExcerptWindows(lines, matcher, sym, fh, callLines, fileSyms = []) {
  let primary;
  if (sym) {
...
import { existsSync as existsSync12, readFileSync as readFileSync13, writeFileSync as writeFileSync7, mkdirSync as mkdirSync8 } from "fs";
...
function excerptsFromText(text, url, title, source, question, perSource) {
  ...
  return excerptWindows(text, question, { perDoc }).map((w) => ({
    source,
    title: w.heading ? `${title} \xA7 ${w.heading}` : title,
    ref: url,
    location: `${url}#~${w.start + 1}`,
    score: Number((w.score + 1).toFixed(3)),
    snippet: w.snippet,
    url,
    meta: w.heading ? { heading: w.heading } : void 0
  }));
}
...
function chunkText(rel2, content, isDoc3, opts = {}) {
  ...
  for (let i2 = 0; i2 < lines.length && chunks.length < maxPerFile; i2 += step) {
    ...
    chunks.push({ rel: rel2, start: i2 + 1, end: Math.min(lines.length, i2 + win), text, isDoc: isDoc3 });
  }
  return chunks;
}
function chunkFile(rel2, content, isDoc3, symbolLines, opts = {}) {
  const win = opts.windowLines ?? 60;
  const maxPerFile = opts.maxPerFile ?? 40;
  const MIN_LEADING = 5;
  const lines = content.split(/\r?\n/);
  ...
  if (isDoc3 || starts.length === 0) return chunkText(rel2, content, isDoc3, opts);
  const chunks = [];
  const add = (from, to) => {
    ...
    chunks.push({ rel: rel2, start: s, end: e, text, isDoc: isDoc3 });
  };
...
async function embedOneChunk(text) {
...
function markerPath(repoDir) {
  return join30(repoDir, ".ultradoc", "semantic.json");
}
...
async function buildIfNeeded(ctx) {
  ...
  if (existsSync12(marker)) {
    try {
      const m = JSON.parse(readFileSync13(marker, "utf8"));
  ...
  for (const rel2 of files) {
    ...
    const content = readText(join30(ctx.repoDir, rel2));
    if (!content) continue;
  ...
  const vectors = await mapLimit(chunks, LIMITS.embedConcurrency, (c2) => embedOneChunk(c2.text));
  ...
  if (!tooHollow) {
    try {
      mkdirSync8(dirname6(marker), { recursive: true });
...
import { existsSync as existsSync13, mkdirSync as mkdirSync9, writeFileSync as writeFileSync8 } from "fs";
...
async function pullStaticModel(opts = {}) {
  ...
  const res = await httpGet(target.url, { timeoutMs: 3e5, retries: 2, maxBytes: 64 * 1024 * 1024 });
  if (!res.ok || !res.body) {
  ...
  if (target.sha256) {
    const got = createHash4("sha256").update(res.body).digest("hex");
  ...
  try {
    mkdirSync9(modelDir(), { recursive: true });
    writeFileSync8(path, res.body);
  } catch (e) {
    return { ok: false, status: "failed", path, message: `ultradoc semantic: could not write the embedding model to ${path} (${e.message}).` };
  }
  return { ok: true, status: "pulled", path, message: `ultradoc semantic: embedding model ready at ${path}` };
}

// src/index/semantic/tiers.ts
function staticEncoder() {
  const model = loadStaticModel();
  if (!model) return void 0;
  return {
    tier: "static",
    buildIndex: async (scan2) => buildEmbeddingIndex(scan2, model),
    encodeQuery: async (query4) => encode(model, query4),
    describe: () => `Semantic search via the local static model (${model.modelId}, no service). It matches symbol names, signatures and file summaries \u2014 not implementation prose. For a "why is it designed this way" question, run \`ultradoc semantic up\` and re-ask: the Docker tier embeds the actual code and docs.`
  };
}
async function endpointEncoder() {
  const base = resolveEmbedEndpoint();
  if (!base) return void 0;
  if (!await probeEndpoint(base)) return void 0;
  return {
    tier: "endpoint",
    buildIndex: async (scan2) => buildEndpointIndex(scan2),
    encodeQuery: async (query4) => encodeQueryViaEndpoint(query4),
...
function staticModelHint() {
  return `run \`ultradoc semantic pull\` to fetch it (~21 MB, once per machine, into ${modelPath()})`;
}

// src/index/semantic/vectors.ts
import { existsSync as existsSync14, mkdirSync as mkdirSync10, readFileSync as readFileSync14, writeFileSync as writeFileSync9 } from "fs";
import { dirname as dirname7, join as join33 } from "path";
function artifactPath(repoDir) {
  return join33(repoDir, CACHE_DIR_NAME, "embeddings.bin");
}
function markerPath2(repoDir) {
  return join33(repoDir, CACHE_DIR_NAME, "embeddings.json");
}
function loadPersisted(ctx, enc) {
  try {
    const marker = JSON.parse(readFileSync14(markerPath2(ctx.repoDir), "utf8"));
    if (marker.tier !== enc.tier || marker.commit !== (ctx.index.commit ?? "HEAD")) return void 0;
    return deserializeEmbeddings(readFileSync14(artifactPath(ctx.repoDir)));
  } catch {
    return void 0;
  }
}
function persist(ctx, enc, index) {
  try {
    mkdirSync10(dirname7(artifactPath(ctx.repoDir)), { recursive: true });
    writeFileSync9(artifactPath(ctx.repoDir), serializeEmbeddings(index));
    writeFileSync9(markerPath2(ctx.repoDir), JSON.stringify({ commit: ctx.index.commit ?? "HEAD", tier: enc.tier, modelId: index.modelId }));
  } catch {
  }
}
function rankRecords(index, queryVec, limit) {
  const scored = [];
  for (const r of index.records) {
...
async function vectorSearch(ctx, enc) {
  const notes = [];
  const scan2 = repoScan(ctx.repoDir);
  let index = loadPersisted(ctx, enc);
  if (!index) {
    index = await enc.buildIndex(scan2);
  ...
  for (const hit of hits) {
    const content = readText(join33(ctx.repoDir, hit.file));
    if (!content) continue;
    const lines = content.split(/\r?\n/);
...
async function semanticSearch(ctx) {
  const tier = ctx.options.semanticTier ?? "auto";
...
async function codeSource(ctx) {
  ...
  const fused = rrf2([lexical.items, sem.items], (it) => it.ref + "@" + (it.location ?? ""));
...
import { existsSync as existsSync15, mkdirSync as mkdirSync11, readFileSync as readFileSync15, statSync as statSync8, writeFileSync as writeFileSync10 } from "fs";
...
async function cachedPageText(dir, url, opts = {}) {
  ...
  try {
    if (existsSync15(file)) {
      cached = readFileSync15(file, "utf8");
      fresh = Date.now() - statSync8(file).mtimeMs < extdocsTtlMs();
    }
  } catch {
  }
  if (cached !== void 0 && fresh) return { text: cached, extractor: asExtractor(planned), finalUrl: url, status: 200 };
  const res = await fetchAndExtract(url, opts);
  if (res.text) {
    const actual = res.extractor ?? EXTRACTOR_NATIVE;
    const out2 = actual === planned ? file : pageCacheFile(dir, url, actual);
    try {
      mkdirSync11(dir, { recursive: true });
      writeFileSync10(out2, res.text);
    } catch {
    }
    return res;
  }
...
async function docsSource(ctx) {
  ...
  for (const rel2 of ctx.index.docFiles) {
    ...
    const inDocsRoot = ctx.index.docsRoot ? rel2.startsWith(ctx.index.docsRoot + "/") : false;
    const boost = (/readme|getting|guide|usage|tutorial/i.test(rel2) ? DOCS_ENTRY_BOOST : 1) * (inDocsRoot ? DOCS_ROOT_BOOST : 1);
    scored.push({ rel: rel2, score: covered.size * 3 * boost + bestHits * 0.5, anchor: bestLine, lines });
  }
  scored.sort((a, b) => b.score - a.score || a.rel.localeCompare(b.rel));
...
function githubReleaseItems(releases, kws) {
  const items = [];
  for (const rel2 of releases ?? []) {
    const text = `${rel2.name ?? ""}
${rel2.body ?? ""}`;
    const cov = coverage(text, kws);
    if (cov === 0) continue;
    const tag = String(rel2.tag_name ?? rel2.name ?? "");
    items.push({
      source: "release",
      title: `Release ${rel2.name || tag}${rel2.published_at ? ` (${String(rel2.published_at).slice(0, 10)})` : ""}`,
      ref: `release:${tag}`,
      location: rel2.html_url,
      score: cov * 3,
      snippet: String(rel2.body ?? "(no release notes)").replace(/\r/g, "").trim().slice(0, 1200),
      url: rel2.html_url,
      meta: { tag, publishedAt: rel2.published_at }
    });
  }
  return items;
}
async function releasesSource(ctx) {
  const notes = [];
  const kws = keywords2(ctx.options.question).map((k) => k.toLowerCase());
  const items = [];
  ...
  for (const rel2 of changelogs) {
    ...
    for (const { s, cov } of scored.slice(0, ctx.options.perSource)) {
      ...
      items.push({
        source: "release",
        title: `${rel2} \u2014 version ${s.version}`,
        ref: `release:${s.version}`,
        location: `${rel2}:${s.start}-${s.start + s.lines.length - 1}`,
        score: cov * 3,
        snippet: s.lines.slice(0, 30).join("\n"),
        url: ctx.repoRef.isLocal ? void 0 : `${ctx.repoRef.webUrl}/blob/${ctx.index.commit ?? "HEAD"}/${rel2}#L${s.start}-L${end}`
      });
    }
  }
...
async function historySource(ctx) {
  ...
  const ranked = rankedKeywords2(ctx.options.question).slice(0, 3);
  if (ranked.length === 0) {
  ...
  const items = [];
  for (const c2 of top) {
    const show = sh2("git", ["-C", ctx.repoDir, "show", "--stat", "-s", "--format=%B", c2.sha], {
      timeoutMs: 3e4
    });
...
async function discussionsSource(ctx) {
  ...
  const ranked = rankedKeywords2(ctx.options.question);
  if (ranked.length === 0) {
...
async function stackoverflowSource(ctx) {
  const kws = rankedKeywords2(ctx.options.question).slice(0, 5).join(" ");
...
async function webSource(ctx) {
  const kws = keywords2(ctx.options.question).slice(0, 8).join(" ");
...
var CODE_SPAN_RE = /`([^`\n]+)`/g;
var QUOTED_RE = /"([^"\n]{3,})"|(?<![A-Za-z])'([^'\n]{3,})'(?![A-Za-z])/g;
function deriveIdentifiers(question) {
  const idents = [];
  const seen = /* @__PURE__ */ new Set();
  const push = (tok) => {
    const t = tok.trim();
...
import { basename as basename5, join as join39 } from "path";

// src/overview.ts
import { existsSync as existsSync16, mkdirSync as mkdirSync12, readFileSync as readFileSync16 } from "fs";
import { basename as basename4, dirname as dirname8, join as join38 } from "path";
...
function ensureOverview(index, ref, repoDir, opts = {}) {
  ...
  if (!opts.refresh && existsSync16(path)) {
    try {
      const existing = readFileSync16(path, "utf8");
...
function renderDocTodo(plan, evidence) {
  ...
  for (const s of plan.sections) {
    ...
    for (const id of s.evidenceIds) {
      ...
      out2.push(`- [${id}] \`${e.ref}\` \u2014 ${firstLine.slice(0, 120)}`);
    }
    out2.push("");
  }
  return out2.join("\n");
...
async function runDoc(options, opts = {}) {
  ...
  writeFileSync12(todoMd, renderDocTodo(plan, evidence));
  writeFileSync12(metaJson, JSON.stringify(meta, null, 2));
...
import { basename as basename6, dirname as dirname9, join as join41, resolve as resolvePath, sep as sep4 } from "path";
...
function isDeclaredUnknown(u) {
  return u.section === UNKNOWNS_SECTION;
}
var UNKNOWNS_SECTION = "unknown";
var UNKNOWNS_HEADING_RE = /^#{1,6}\s+(unknowns?|not settled|open questions?|gaps?)\b/i;
function claimUnitsOf(text) {
  return extractClaimUnits(text, {
    blockquotes: "prose",
    keepInlineCode: true,
    skipTableHeader: false,
    sectionTag: (heading) => UNKNOWNS_HEADING_RE.test(heading) ? UNKNOWNS_SECTION : void 0
  });
}
function citationsIn(text) {
  return citationTokensIn(text, isCitation);
}
...
function collectCitationTokens(text) {
  const { grounding, inertOnly } = collectCitations(text, isCitation, {
    blockquotes: "prose",
    keepInlineCode: true,
    skipTableHeader: false,
    sectionTag: (heading) => UNKNOWNS_HEADING_RE.test(heading) ? UNKNOWNS_SECTION : void 0
...
// src/verify.ts
import { existsSync as existsSync17, readFileSync as readFileSync17, writeFileSync as writeFileSync13 } from "fs";
...
function buildWorklist(dir, opts = {}) {
  ...
  if (!existsSync17(evidencePath)) throw new Error(`No evidence.json in ${dir} \u2014 run \`ultradoc ask\` first.`);
  const evidence = JSON.parse(readFileSync17(evidencePath, "utf8"));
  const byId = new Map(evidence.map((e) => [e.id, e]));
...
function runVerify(dir, opts = {}) {
  ...
  writeFileSync13(join40(dir, "VERIFY.todo.json"), JSON.stringify(todo, null, 2));
  writeFileSync13(join40(dir, "VERIFY.md"), renderWorklistMd(worklist, total, kept));
  return worklist;
}
...
function applyVerdicts(dir, verdictsPath) {
  ...
  writeFileSync13(join40(dir, "VERIFY.json"), JSON.stringify({ ...result, verdicts, ...answerSig ? { answerSig } : {}, claims }, null, 2));
  return result;
}
function expectedClaims(dir) {
  try {
    const todoPath = join40(dir, "VERIFY.todo.json");
    if (!existsSync17(todoPath)) return null;
...
function answerSignatureFor(dir) {
  try {
    ...
    const evidencePath = join40(dir, "evidence.json");
    if (!answerPath || !existsSync17(evidencePath)) return null;
    const evidence = JSON.parse(readFileSync17(evidencePath, "utf8"));
    return answerClaimSignature(readFileSync17(answerPath, "utf8"), evidence);
  } catch {
    return null;
  }
}
...
function pinnedClone(dir) {
  ...
  try {
    ...
    if (!existsSync18(metaPath)) return pin;
    const meta = JSON.parse(readFileSync18(metaPath, "utf8"));
    pin.meta = meta;
    if (!meta.commit) return pin;
    pin.recordedRepoDir = meta.repoDir;
    const repoDir = meta.repoDir && existsSync18(meta.repoDir) ? meta.repoDir : dossierRepoDir(dir);
    if (!repoDir) return pin;
    pin.repoDir = repoDir;
    const head = headCommit2(repoDir);
    if (!head) return pin;
    pin.head = head;
...
function revalidateEvidence(pin, evidence, errors, warnings) {
  ...
  for (const item of candidates) {
    ...
    let lines;
    try {
      lines = readFileSync18(abs, "utf8").split(/\r?\n/);
    } catch (e) {
...
function missingDocSections(dir, answerPath, answer) {
  ...
  if (!existsSync18(planPath)) return void 0;
  let plan;
  try {
...
function checkRun(dir, opts = {}) {
  ...
  let evidence;
  try {
    evidence = JSON.parse(readFileSync18(evidencePath, "utf8"));
  } catch (e) {
    return {
      ok: false,
      citations: [],
  ...
  const answer = opts.answerText ?? readFileSync18(answerPath, "utf8");
  const ids = new Set(evidence.map((e) => e.id));
...
function symbolEvidence(ctx, name2, opts = {}) {
  const max = opts.max ?? ctx.options.perSource;
  const notes = [];
  const items = [];
  const scan2 = repoScan(ctx.repoDir);
  const inScope = (rel2) => !ctx.scopeDir || rel2.startsWith(`${ctx.scopeDir}/`);
  const symsByFile = symbolsByFile(ctx.index.symbols);
  ...
  const defs = findSymbol(scan2, name2, { maxResults: max }).filter((d) => inScope(d.file));
  if (!defs.length) {
    const near = findSymbol(scan2, name2, { substring: true, maxResults: 8 }).filter((d) => inScope(d.file)).map((d) => `${d.name} (${d.file}:${d.line})`);
    notes.push(
      near.length ? `No declaration named "${name2}". Similar names in this repo: ${near.join(", ")}.` : `No declaration named "${name2}" \u2014 check the spelling, or drill with \`code --q "${name2}"\` if it is not a declared symbol (a config key, a string literal, a callback property).`
    );
  }
    ...
    items.push(
      codeItem({
        ...
        meta: {
          ...
          ...def.endLine !== void 0 && (w.start > def.line || w.end < def.endLine) ? { symbolSpan: `${def.line}-${def.endLine}` } : {}
        }
      })
    );
  }
  ...
  for (const site of callers) {
    ...
    const host = enclosingSymbol2(symsByFile.get(site.file) ?? [], site.line);
    ...
    items.push(
      codeItem({
        ...
        label: host ? `calls ${name2} \u2014 from ${symbolLabel(host)}` : `calls ${name2}`,
        score: CALL_SCORE,
        meta: {
          symbol: name2,
          callSite: true,
...
// src/cache.ts
import { existsSync as existsSync19, readdirSync as readdirSync5, rmSync as rmSync4, statSync as statSync9 } from "fs";
...
function cacheStatus() {
  ...
  try {
    slugs = readdirSync5(root).filter((n) => {
      try {
        return statSync9(join43(root, n)).isDirectory();
      } catch {
        return false;
      }
    });
...
function cleanRepoCache(opts) {
  ...
  if (opts.all) {
    for (const r of cacheStatus().repos) {
      try {
        rmSync4(r.dir, { recursive: true, force: true });
        removed.push(r.slug);
      } catch {
      }
    }
...
var DRILL = {
  ...
  ids: (plan) => Array.isArray(plan?.cells) ? plan.cells.map((c2) => c2.id) : void 0,
  prerequisite: (run3, engineAbs) => `node ${engineAbs} ask --repo <url|path> --q "<question>" --out ${run3}`,
  role: "explorer",
  title: "Drill",
  schema: DRILL_SCHEMA,
  batchSize: 8,
  description: (n) => `Fan out the ${n} retrieval drill cell(s) of an ultradoc run (explorer fan-out, triaged returns)`,
  applyHint: (run3, engineAbs) => [`node ${engineAbs} check --run ${run3} --strict`]
};
var VERIFY = {
  name: "verify",
  worklist: "VERIFY.todo.json",
  ids: (todo) => Array.isArray(todo?.pairs) ? todo.pairs.map((p) => `${p.claimId}:${p.evidenceId}`) : void 0,
  prerequisite: (run3, engineAbs) => `node ${engineAbs} verify --run ${run3}`,
  role: "skeptic",
  title: "Verify",
  schema: VERIFY_SCHEMA,
  batchSize: 8,
  description: (n) => `Adversarially verify the ${n} claim\u2194evidence pair(s) of an ultradoc answer (skeptic fan-out)`,
  applyHint: (run3, engineAbs) => [`node ${engineAbs} verify --apply verdicts.json --run ${run3} && node ${engineAbs} check --run ${run3} --semantic`]
};
var DOC = {
  name: "doc",
  worklist: "DOC.plan.json",
  ids: (plan) => Array.isArray(plan?.sections) ? plan.sections.map((s) => s.id) : void 0,
  prerequisite: (run3, engineAbs) => `node ${engineAbs} doc --repo <url|path> --out ${run3}`,
  role: "section-writer",
  title: "Write",
  schema: DOC_SCHEMA,
  batchSize: 8,
  description: (n) => `Draft the ${n} outline section(s) of an ultradoc reference doc (section-writer fan-out)`,
  applyHint: (run3, engineAbs) => [`node ${engineAbs} check --run ${run3}`]
};
var PHASE_DEFS = [DRILL, VERIFY, DOC];
function listPhasesFor(runDir2, engineAbs) {
  return listPhases(runDir2, engineAbs, PHASE_DEFS);
}
function emitOrchestration(runDir2, engineAbs, opts = {}) {
  return orchestrateRun(runDir2, engineAbs, PHASE_DEFS, agentContracts, {
    ...opts,
    runbookPreamble: runbookPreamble(listPhasesFor(runDir2, engineAbs), runDir2, engineAbs)
  });
}

// src/mcp/handlers.ts
import { existsSync as existsSync20, readFileSync as readFileSync19, realpathSync as realpathSync3, statSync as statSync10 } from "fs";
import { isAbsolute as isAbsolute2, resolve as resolve4, sep as sep5 } from "path";
...
function askOptions(args2, defaults, opts = {}) {
  ...
  const docsUrl = str2(args2.docs_url);
  ...
  return {
    ...
    question: opts.question ?? str2(args2.question) ?? "",
    sources,
    ref: str2(args2.ref),
    docsUrl,
    pkg: str2(args2.package),
    out: out2,
...
async function handleOverview(args2, defaults, warmNotes2) {
...
async function handleSymbol(args2, defaults, warmNotes2) {
  ...
  const ctx = await context(askOptions(args2, defaults, { question: name2 }));
...
async function handleRead(args2, defaults) {
  const rel2 = requiredStr(args2, "path", "a repo-relative path, or an absolute path inside the clone or ultradoc's cache.");
  const ctx = await context(askOptions(args2, defaults, { question: "" }));
  const root = realpathSync3(ctx.repoDir);
  const target = isAbsolute2(rel2) ? rel2 : resolve4(root, rel2);
  let real;
  try {
  ...
  if (stat.isDirectory()) throw new ToolError(`\`path\` is a directory, not a file: ${rel2}`);
  const wantsWindow = args2.start_line !== void 0 || args2.end_line !== void 0;
  if (stat.size > WINDOWED_READ_MAX_BYTES) {
    throw new ToolError(
      `${rel2} is ${stat.size} bytes \u2014 too large for ultradoc to open (ceiling ${WINDOWED_READ_MAX_BYTES}). Read it with your own file tools.`
    );
  }
  ...
  if (start2 > total) throw new ToolError(`start_line ${start2} is past the end of ${rel2} (${total} lines).`);
...
async function handleFetch(args2) {
  ...
  const perSource = num2(args2.per_source);
  if (perSource !== void 0 && perSource <= 0) throw new ToolError("`per_source` must be greater than 0.");
  const question = str2(args2.question) ?? "";
...
function handleCheck(args2) {
  ...
  const result = checkRun(dir, {
    answerText,
    answerFile: str2(args2.answer_file),
...
function handleVerify(args2) {
  ...
  try {
    return { ...runVerify(dir, { maxVerify, answerFile: str2(args2.answer_file) }), run_dir: dir };
  } catch (e) {
    throw new ToolError(errMessage3(e));
  }
}
...
function parseCli(argv) {
  let parsed;
  try {
    parsed = parseArgs(argv, { commands: COMMANDS, valueFlags: VALUE_FLAGS2, boolFlags: BOOL_FLAGS });
  } catch (e) {
...
async function run2(argv = process.argv.slice(2)) {
  ...
  switch (p.command) {
    ...
    case "symbol": {
      const name2 = p.values.name;
      if (!name2) fail("missing --name <symbol> (e.g. --name retryRequest)");
      const opts = buildAskOptions({ ...p, values: { ...p.values, q: name2 } });
      const ctx = await buildContext(opts);
      const { items, notes } = symbolEvidence(ctx, name2);
      const evidence = assignIds2([{ source: "code", items, notes }]);
      const meta = {
        question: `symbol: ${name2}`,
        repo: ctx.repoRef.raw,
        host: ctx.repoRef.host,
        ref: opts.ref,
        commit: ctx.index.commit,
...
`);
        ...
        process.stdout.write(JSON.stringify({ phases: listPhasesFor(resolve5(dir), engineAbs) }, null, 2) + "\n");
        return;
      }
      const res = emitOrchestration(resolve5(dir), engineAbs, {
...
`);
        ...
        process.stdout.write(`Follow ${join45(resolve5(dir), "orchestration", "RUNBOOK.md")} sequentially (the eco path).
`);
...
`);
    ...
    case "semantic": {
      ...
      if (action === "pull") {
        const r2 = await pullStaticModel({ force: p.bools.has("refresh") });
        process.stdout.write(r2.message + "\n");
