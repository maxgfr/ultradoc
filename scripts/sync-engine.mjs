...
//
// Both are self-contained zero-dependency ESM bundles; tsup inlines them into
// scripts/ultradoc.mjs at build time, so the skill still ships as one file.
//
...
const ENGINES = {
  ...
  webindex: {
    ...
    // v1.12.1: src/index/semantic/control.ts drives the stack through the
    // engine's stackControl with a service ARRAY (["semantic","searxng"]), which
    // is the v1.12.0 folded call, plus the v1.12.1 fix that materialises the
    // compose under <PREFIX>_CACHE_DIR. It sat at v1.6.0 while the pin was
    // v1.12.1 — six releases the STALE check could not fire on.
    minRef: "v1.12.1",
    ...
    files: [
      ...
      // these, a bundler does not. Before this, `web-discovery.md` and
      // `provider-apis.md` existed in two skills with different bytes, both
      // describing one engine.
      { remote: "references/web-discovery.md", local: "web-discovery.md", dest: "skills/ultradoc/references/web-discovery.md" },
      { remote: "references/provider-apis.md", local: "provider-apis.md", dest: "skills/ultradoc/references/provider-apis.md" },
    ],
  },
};
