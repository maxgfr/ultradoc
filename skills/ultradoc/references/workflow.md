...
For engine evidence and coverage limits, read [engine-evidence](engine-evidence.md).
...
| tune caps, cost, cache | `--help` · `./tuning.md` · `cache status\|clean` |

For `trace`, read `./trace.md`: budgets, ambiguity stops and coverage
...
every drill after the first `ask` is near-free (`./tuning.md`).
...
| huge repo, slow index / a "truncated" note | scope with `--package`; raise a cap only per `./tuning.md` |
| the **question** is ambiguous (which repo? which behavior?) | ask the user before retrieving — never guess the repo |
| the dossier is empty/off-topic twice in a row | re-phrase per `./retrieval-playbook.md`; it's the wording, not a missing answer |
...
2. **Retrieve.** Derive 2–3 **query variants** — the engine searches literally,
   ...
   source tables are in `./retrieval-playbook.md`.
...
3. **Read the dossier.** **Retrieval notes first** — they say what this run could
   ...
   titles, then the snippets. `./reading-evidence.md` decodes every
...
5. **Write the answer** to `ANSWER.md` in the run folder, per **The answer
   contract** below and `./citation-format.md`.
...
6. **Validate (two layers).**
   ...
   - *Semantic (adversarial support-check):* `verify --run <dir>` writes a
     ...
     `verdicts.json` (`./orchestration.md` has the verdict table and the
     ...
     and re-verify. Then self-review against `./answer-rubric.md`.

7. **Present** per the answer contract. `./worked-example.md` walks one
...
3. **Validate & present.** `check --run <doc-dir> --strict` (and `verify` +
...
noting any failure. See `./web-discovery.md`.

## References

| Open it when | File |
|---|---|
| the dossier is in front of you: notes, item labels, gate warnings | `./reading-evidence.md` |
| choosing sources, phrasing variants, iterating, triaging | `./retrieval-playbook.md` |
| writing citations, or `check` rejected one | `./citation-format.md` |
| the answer is drafted and you're about to present | `./answer-rubric.md` |
| you want the whole loop demonstrated once | `./worked-example.md` |
| a run is too slow, too shallow, or too noisy | `./tuning.md` |
| parallelizing drills or verification across calls/subagents | `./orchestration.md` |
| issues/PRs look wrong or empty for a host | `./provider-apis.md` |
| `web` found nothing, or a page extracted badly | `./web-discovery.md` |
| setting up a vector tier or the Firecrawl stack | `./semantic-setup.md` |
