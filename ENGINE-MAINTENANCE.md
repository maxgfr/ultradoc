...
The daily workflow and development CLI are pinned by immutable commits. The CLI follows stable releases automatically. A maintainer updates the workflow reference separately: GitHub’s automatic token cannot modify workflow definitions. It validates even when no pin changes, never rebases a tested candidate, and waits for CI and publication. `pnpm exec webindex skill finish` resumes incomplete publication on the current main commit. Runtime bundles remain self-contained; the maintenance package is development-only.

Import floors count symbols from the actual engine import. Shared names imported from WebIndex do not count as CodeIndex adoption; the initial CodeIndex floor excludes those overlaps. No source imports were removed by this measurement correction.
