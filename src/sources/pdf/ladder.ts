...
/**
 * The rungs to try, honouring `ULTRADOC_PDF_ENGINE` (force exactly one) and
 * `ULTRADOC_NO_NPX` (skip the rung that needs an implicit install).
 *
...
export function enabledExtractors(engines?: PdfExtractorId[]): PdfExtractorId[] {
  ...
  const forced = process.env.ULTRADOC_PDF_ENGINE?.trim() as PdfExtractorId | undefined;
...
export async function extractPdf(bytes: Buffer, opts: PdfLadderOptions = {}): Promise<PdfExtraction> {
  ...
  for (const id of enabledExtractors(opts.engines)) {
    ...
    // the PDF instead of raising ULTRADOC_OCR_MAX.
