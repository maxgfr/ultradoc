...
export function createServer(opts: ServerOptions = {}): McpServer {
  ...
  const listTools = () => toolsFor(protocol, { defaultRepo: opts.defaultRepo, allowWrite: opts.allowWrite });
  ...
  async function handleToolCall(msg: JsonRpcMessage, reply: (out: Omit<JsonRpcMessage, "jsonrpc" | "id">) => void): Promise<void> {
    ...
    try {
      const { text: raw, artifact } = await callTool(name, args, { defaultRepo: opts.defaultRepo, allowWrite: opts.allowWrite });
